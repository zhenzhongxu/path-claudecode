...
- `system:session-end` — Session end bookend (hook-produced)
- `perception:situation` — User prompt capture (hook-produced)
- `modification:applied` — Rule file changes (hook-produced)
- `modification:proposal` — Proposed change before applying (agent-produced, /evolve)
- `feedback:human` — Feedback input logged (agent-produced, /evolve)
- `evolve:analysis` — Gap analysis results (agent-produced, /evolve)
- `reflection:analysis` — Deep reflection results (agent-produced, /reflect)
- `system:export` — State snapshots (agent-produced, /export-state)
...
### Step 5b: Log Reflection Event

After presenting the report, emit a `reflection:analysis` event to the event log. This is observability (append-only log), not state mutation — it does not violate the read-only constraint.

```bash
PROMPT_VERSION=$(jq '.cycleCount // 0' .claude/path-kernel/state.json 2>/dev/null || echo 0)
bash .claude/hooks/append-event.sh "reflection:analysis" \
  '{"summary":"<themes identified>","recommendations_count":<N>}' "$PROMPT_VERSION"
```

Replace `<themes identified>` with a brief summary of themes from Step 3, and `<N>` with the number of recommendations from Step 4.

## Constraints

- This skill is **read-only** with respect to rules and state files. Do NOT modify any rules, state, or config files.
- Emitting events to the append-only event log (via `append-event.sh`) is permitted — it is observability, not state mutation.
